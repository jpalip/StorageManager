...
public class SlottedPage implements Iterable<Object> {
	...
	public int add(Object o) throws IOException, OverflowException {
		...
		saveLocation(count, location);

		// Print out each byte in the array
//		System.out.print("[ ");
...
//		System.out.print("]");
	...
	public Object remove(int index) throws IndexOutOfBoundsException, IOException {
		...
		if (location == -1)
			return null;
		Object old = toObject(data, location);
...
//		System.out.print("]");
	...
	protected void compact() throws IOException {
		...
		SlottedPage newPage = new SlottedPage(0, data.length);
		for (Object o : this) {
			try {
				newPage.add(o);
		...
		System.arraycopy(newPage.data, 0, data, 0, data.length);
		this.pageID = newPage.pageID;
